...
Для работы экспортера в режиме `internal` (внутреннее хранение секретов) и для самообновления используются настройки GitLab. Теперь для всех операций (запуск пайплайнов, чтение релизов) применяется единый **Personal Access Token** с правами `api`.
...
| `AccessToken` | ✅ | Personal Access Token с правами `api`. Используется для запуска пайплайнов (через заголовок `PRIVATE-TOKEN`) и для доступа к релизам при самообновлении. |
...
- Токен должен быть создан в GitLab с правами `api`. Он будет использоваться как для инициации пайплайнов (чтобы экспортер мог запросить секреты), так и для скачивания новых версий при самообновлении.
...
Создайте службу через `sc` и настройте её перезапуск при сбое:
